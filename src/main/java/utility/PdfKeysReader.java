...
public class PdfKeysReader extends PDFTextStripper {
	public static List<String> words = new ArrayList<String>();
	Map<Object, String> pdfData = new HashMap<Object, String>();

	public PdfKeysReader() throws IOException {
	...
	public void read(File file) throws IOException {
		...
		try {
			...
			PDFTextStripper stripper = new PdfKeysReader();
		...
		for (int i = 0; i < words.size(); i++) {
			...
			case "PAN":
				if (words.get(i + 1).equals("No") && (words.get(i + 2).equals(":"))) {
					pdfData.put(Keys.PANNo, words.get(i + 3));

				}
				break;
			case "Invoice":
				if (words.get(i + 1).equals("No") && (words.get(i + 2).equals(":"))) {
					pdfData.put(Keys.InvoiceNo, words.get(i + 3));
				}
				break;
			case "Invoice/Payment":
				if (words.get(i + 1).equals("Date") && (words.get(i + 2).equals("&"))
						&& (words.get(i + 3).equals("Time")) && (words.get(i + 4).equals(":"))) {
					pdfData.put(Keys.InvoicePaymentDateAndTime,
			...
			case "Invoice/Payment2":
				if (words.get(i + 1).equals("Date") && (words.get(i + 2).equals("&"))
						&& (words.get(i + 3).equals("Time")) && (words.get(i + 4).equals(":"))) {
					pdfData.put(Keys.Invoice_Payment,
							words.get(i + 5).concat(" ").concat(words.get(i + 6)).concat(" ").concat(words.get(i + 7)));
				}
				break;
			default:
